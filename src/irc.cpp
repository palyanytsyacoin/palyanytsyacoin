...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("PalyanytsyaCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #PalyanytsyaCoinTEST\r");
            Send(hSocket, "WHO #PalyanytsyaCoinTEST\r");
        } else {
            // randomly join #PalyanytsyaCoin00-#PalyanytsyaCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 1 for initial release
            int channel_number = 1;
            Send(hSocket, strprintf("JOIN #PalyanytsyaCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #PalyanytsyaCoin%02d\r", channel_number).c_str());
        }
