...
unsigned int nStakeTargetSpacing = nOneDay; // 1 day stakes spacing
...
const string strMessageMagic = "PalyanytsyaCoin Signed Message:\n";
...
{
    if(nHeight >= 1 && nHeight <= 10) return 100000000 * COIN;
    if(nHeight >= 11 && nHeight <= 1011) return 800000 * COIN;
    if(nHeight >= 1012) return 1 * COIN;
    ...
    // PalyanytsyaCoin: subsidy is cut in half every 64x multiply of PoW difficulty
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nTime, bool bCoinYearOnly, int nHeight)
{
    ...
    if (Balance < (999999 * COIN)) {
        nRewardCoinYear = 2 * CENT;
    } else if (Balance >= (1000000 * COIN) &&  Balance <= (2222222 * COIN)) {
        nRewardCoinYear = 3 * CENT;
    } else if (Balance >= (2222222 * COIN) &&  Balance <= (3333333 * COIN)) {
        nRewardCoinYear = 4 * CENT;
    } else if (Balance >= (3333333 * COIN) &&  Balance <= (4444444 * COIN)) {
...
static const int64_t nTargetTimespan =  3 * nOneDay;  // 3 day
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("PalyanytsyaCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in PalyanytsyaCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // PalyanytsyaCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PalyanytsyaCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "PalyanytsyaCoin(PLCS)";
        unsigned int nTimeGenesis = 1645666800;
        unsigned int nNonceGenesis = 545806;

        if (fTestNet)
        {
            pszTimestamp ="PalyanytsyaTestCoin(PLCS)";
            nTimeGenesis = 1514267180;
            nNonceGenesis = 103907;
        }
        ...
        if (!fTestNet)
            assert(block.hashMerkleRoot == uint256("0xfa2466e028b46d6fc201cb83fc49ca2e570d8ba42c51c4da7e05668a71e7cf37"));
        else
            assert(block.hashMerkleRoot == uint256("0x610910135afb697a15e70fd8a62135ca3b9ce5c20bea8c15c75fdd422c300cd7"));
...
unsigned char pchMessageStart[4] = { 0x61, 0x46, 0x2b, 0xad };
