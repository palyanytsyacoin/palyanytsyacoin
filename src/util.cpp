...
#else
    const char* pszModule = "PalyanytsyaCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PalyanytsyaCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PalyanytsyaCoin
    // Mac: ~/Library/Application Support/PalyanytsyaCoin
    // Unix: ~/.PalyanytsyaCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PalyanytsyaCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "PalyanytsyaCoin";
#else
    // Unix
    return pathRet / ".PalyanytsyaCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "PalyanytsyaCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "palyanytsyacoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong PalyanytsyaCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("PalyanytsyaCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
